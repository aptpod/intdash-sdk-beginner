...
class Downstreamer:
    ...
    async def read_basetime(
        self,
    ) -> AsyncGenerator[Tuple[str, iscp.DateTime, int], None]:
        """
        基準時刻受信
        ...
        Yields:
            (str, DateTime, int): セッションID, 基準時刻, priority
        """
        async for metadata in self.down.metadatas():
            ...
            yield (
                ...
                metadata.metadata.base_time,
                metadata.metadata.priority,
            )

    async def read(self) -> AsyncGenerator[Tuple[str, bool, int, bytes], None]:
        """
        データチャンク受信

        Yields:
            tuple(str, bool, int, bytes): セッションID, IDRフレーム判定,
                データポイントの経過時間, ペイロード
        """
        async for msg in self.down.chunks():
            for group in msg.data_point_groups:
                is_idr = group.data_id.type == "h264_frame/idr_frame"
                for data_point in group.data_points:
                    yield (
                        msg.upstream_info.session_id,
                        is_idr,
