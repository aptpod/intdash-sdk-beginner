...
def main(
    ...
    """
    logging.info(
        f"Processing project_uuid: {project_uuid} meas_uuid: {meas_uuid}, edge_uuid: {edge_uuid} start: {start} end: {end} outdir: {outdir} tracks: {','.join(tracks)} fps: {fps} gmap_api_key: {gmap_api_key} mux: {mux}"
    )
    service = None
    try:
